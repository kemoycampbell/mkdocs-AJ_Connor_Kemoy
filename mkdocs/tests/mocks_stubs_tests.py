...
class TestMocksAndStubs(unittest.TestCase):
    def test_clean_directory_with_hidden_files(self):
        ...
        with patch('os.path.exists', return_value=True), patch(
            'os.listdir', return_value=['.hidden', '.gitignore', '.DS_Store']
        ) as mock_listdir, patch('shutil.rmtree') as mock_rmtree, patch('os.unlink') as mock_unlink:
    ...
    def test_get_deps_command(self):
        ...
        with patch('mkdocs_get_deps.get_deps') as mock_get_deps:
            mock_get_deps.return_value = [
                "some arbitrary val representing a plugin"
            ]  # Simulate successful execution
            result = runner.invoke(
                cli.cli, ["get-deps", "-f", "mkdocs.yml"], catch_exceptions=False
            )
    ...
    def test_get_deps_command_exit_if_warnings(self):
        ...
        runner = CliRunner()
        with patch('mkdocs.utils.CountHandler') as mock_count_handler, patch(
    ...
    def test__load_hook_fail_spec_from_file(self):
        ...
        with patch('importlib.util') as mock_import_util:
            mock_import_util.spec_from_file_location.return_value = (
                None  # Simulate failure to load spec
            )
    ...
    def test__load_hook_fail_module_from_spec(self):
        ...
        with patch('importlib.util') as mock_import_util:
            ...
            mock_import_util.module_from_spec.return_value = (
                MagicMock()
            )  # Simulate failure to load module
            hooks = Hooks("some_fake_plugin_key")
            with self.assertRaises(ValidationError):
                hooks._load_hook("non_existent_hook", "non_existent_hook.py")
    ...
    def test__is_cwd_git_repo_no_git(self):
        ...
        with patch.object(logger, 'error') as mock_log_error, patch(
            'subprocess.Popen'
        ) as mock_popen:
    ...
    def test_serve_function(self, mock_load_config, mock_build):
        ...
        mock_config = MagicMock(
            dev_addr=('somehost', 8000), site_url=expected_site_url, watch=['mkdocs']
        )
        ...
        with patch.object(logger, 'info') as mock_log_info, patch(
            'tempfile.mkdtemp', return_value=expected_fake_dir
        ), patch('mkdocs.commands.serve.LiveReloadServer', return_value=mock_server), patch(
            'mkdocs.commands.serve.isdir', return_value=True
        ), patch(
            'shutil.rmtree'
        ) as mock_rmtree:
    ...
    def test_serve_function_with_livereload(self, mock_load_config, mock_build):
        ...
        with patch.object(logger, 'info') as mock_log_info, patch(
            ...
            mock_log_info.assert_called_once_with("Building documentation...")
            mock_build.assert_called_once_with(mock_config, serve_url=None, dirty=False)
            mock_server.watch.assert_has_calls(
                [
                    call(i)
                    for i in ['docs/', 'mkdocs.yml', 'some_theme_dir', 'mkdocs', 'passed-in-dir/']
                ]
            )
